...
通过自定义模型，可以修改或者新增自己的数据行为。通过继承 `ChatServiceImpl` 并重载方法，可以实现更灵活的业务逻辑。

```typescript
// 1. 集成ChatServiceImpl接口，实现自定义接口
...
通过重载接口，可以在数据操作时触发自定义事件，让组件接收特定通知。
...
function MyCustomComponent() {
  ...
  React.useEffect(() => {
    const handleBeforeSend = (data: any) => {
      ...
      // 可以在这里执行自定义逻辑，如显示加载状态
    };

    const handleAfterSend = (data: any) => {
      console.log("消息发送成功:", data);
      // 可以在这里执行自定义逻辑，如隐藏加载状态、显示提示
    };
...
通过重载接口，可以对返回的数据进行过滤或处理，满足特定业务需求。
